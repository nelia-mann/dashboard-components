...
`]}customElements.define("volume-slider",it);let ie=tm(class extends tf{constructor(t){if(super(t),3!==t.type&&1!==t.type&&4!==t.type)throw Error("The `live` directive is not allowed on child or event bindings");if(void 0!==t.strings)throw Error("`live` bindings can only contain a single expression")}render(t){return t}update(t,[e]){if(e===U||e===G)return e;let i=t.element,s=t.name;if(3===t.type){if(e===i[s])return U}else if(4===t.type){if(!!e===i.hasAttribute(s))return U}else if(1===t.type&&i.getAttribute(s)===e+"")return U;return tS(t),e}});class ii extends tN{static properties={...super.properties,_value:{state:!0}};constructor(){super(),this._value=0}getTriggers(){return["_value"]}updated(){this.getChangeFlag()||setTimeout(()=>this.setInitialValues(),1e3)}setInitialValues(){this.getChangeFlag()||this.setValue(this.getTrackPosition())}getValue(){return this._value}setValue(t){this._value=t}getSpeakerId(){return[...this.getEntityIds()][0]}getSpeakerState(){return this.getStates()[this.getSpeakerId()]}getSpeakerAttributes(){return this.getSpeakerState().attributes}getTrackLength(){return this.getAttribute(this.getSpeakerId(),"media_duration")}getTrackTitle(){return this.getAttribute(this.getSpeakerId(),"media_title")}getTrackUpdated(){return new Date(this.getAttribute(this.getSpeakerId(),"media_position_updated_at")).getTime()}isPlaying(){return"playing"===this.getStateState(this.getSpeakerId())}getTrackPosition(){let t=this.getTrackUpdated(),e=this.getAttribute(this.getSpeakerId(),"media_position"),i=e+(Date.now()-t)/1e3;return this.isPlaying()?i<this.getTrackLength()?Math.round(i):Math.round(this.getTrackLength()):Math.round(e)}formatTime(t){let e=Math.floor(t/60),i=String(t-60*e);return 1===i.length&&(i="0"+i),String(e)+":"+i}isTrackTimeUpdated(t,e){return e!==this.getAttribute(t,"media_position_updated_at")}isTrackTimeUpdated(t,e){let i=this.getAttribute(t,"media_position");return console.log(i,e),i<=Number(e)+1&&Number(e)-1<=i}handleOnInput(t){this.raiseChangeFlag(),this.setValue(Number(t.target.value))}async handleOnChange(t){let e=t.target.value,i={entity_id:this.getSpeakerId(),seek_position:e};this.getAttribute(this.getSpeakerId(),"media_position_updated_at"),this.callService("media_player","media_seek",i),await this.waitForEntity(this.getSpeakerId(),t=>this.isTrackTimeUpdated(t,e)),this.lowerChangeFlag(),this.requestUpdate()}render(){if(this.isInitialized())return N`
    ...
    `]}customElements.define("track-slider",ii);class is extends tN{getSpeakerId(){return[...this.getEntityIds()][0]}getSpeakerState(){return this.getStateState(this.getSpeakerId())}isPlaying(){return"playing"===this.getSpeakerState()}getPlayPauseIcon(){return this.isPlaying()?"M14,19H18V5H14M6,19H10V5H6V19Z":"M8,5.14V19.14L19,12.14L8,5.14Z"}handlePrevious(){let t=this.getSpeakerId();t&&this.callService("media_player","media_previous_track",{entity_id:t})}handlePlayPause(){let t=this.getSpeakerId();t&&this.callService("media_player","media_play_pause",{entity_id:t})}handleNext(){let t=this.getSpeakerId();t&&this.callService("media_player","media_next_track",{entity_id:t})}previous(){return N`<div class = "outlined button" @click=${this.handlePrevious}> 
