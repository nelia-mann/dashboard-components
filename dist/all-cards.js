...
`]}customElements.define("player-panel",ir);class ia extends e6{static properties={...super.properties,entityIds:{state:!0},selected:{type:Boolean}};getTriggers(){return["entityIds","selected"]}updated(t){super.updated(t),Object.assign(this.style,this.getStyles())}getRGB(t){return this.isPlaying()?t5(t2(tZ,tX,this.getAverageVolume()),t):t5(tZ,t)}isSelected(){return this.selected}handlePointerDown(t,e){this._debugLog("handlePointerDown inside group-panel fires");let i=t.currentTarget.getBoundingClientRect();this.dispatchEvent(new CustomEvent("speaker-drag-start",{detail:{speakerId:e,pointerId:t.pointerId,clientX:t.clientX,clientY:t.clientY,width:i.width,height:i.height}}))}speakerTile(t){return N`
    ...
    `]}customElements.define("speaker-group-panel",ia);class io extends e6{_ghost;_suppressDrag;_draggedSpeakerId;static properties={...super.properties,players:{state:!0},selectedIndex:{state:!0}};constructor(){super(),this.players=[],this._ghost=null,this.selectedIndex=null,this._suppressDrag=!1}getTriggers(){return["players","selectedIndex"]}onFirstUpdate(){this.initializeSelectedIndex()}setInitialValues(){this.initializePlayers()}initializePlayers(){let t=[];this.getSpeakerIds().forEach(e=>{if(this.isAlone(e))t.push([e]);else{let i=this.getGroup(e);i[0]===e&&t.push(i)}}),this.setPlayers(t)}initializeSelectedIndex(){let t=0;this.getPlayers().forEach((e,i)=>{let s=this.getPlayer(t).length;e.length>s&&(t=i)}),this.setSelectedIndex(t)}getSelectedIndex(){return this.selectedIndex}setSelectedIndex(t){this.selectedIndex=t;let e=this.getLeaderFromIndex(t);this.dispatchEvent(new CustomEvent("select",{detail:e}))}isSelected(t){return t===this.getSelectedIndex()}getSuppressState(){return this._suppressDrag}raiseSuppress(){this._suppressDrag=!0}lowerSuppress(){this._suppressDrag=!1}getDraggedId(){return this._draggedSpeakerId}setDraggedId(t){this._draggedSpeakerId=t}getPlayers(){return[...this.players]}setPlayers(t){this.players=t}getPlayer(t){return[...this.getPlayers()[t]]}getPlayerIndex(t){let e=[];return this.getPlayers().forEach((i,s)=>{i.includes(t)&&e.push(s)}),e[0]}checkGroup(t,e){return t.length===e.length&&t.every(t=>e.includes(t))}isUnjoined(t,e){let i=this.getPlayerIndex(e),s=this.getPlayer(i);return s.every(t=>this.checkGroup(s,this.getGroup(t)))&&this.isAlone(t)}isJoined(t,e){let i=this.getPlayerIndex(e),s=this.getPlayer(i);return s.every(t=>this.checkGroup(s,this.getGroup(t)))}isLeader(t){let e=this.getGroup(t);return 0!==e.length&&t===e[0]}getLeader(t){let e=this.getGroup(t);return 0===e.length?t:e[0]}getLeaderFromIndex(t){return this.getPlayer(t)[0]}removeFromPlayer(t){let e=this.getPlayerIndex(t),i=this.getPlayer(e).filter(e=>e!==t),s=this.getPlayers();if(i.length>0?s[e]=i:s.splice(e,1),this.setPlayers(s),i.length>0)return i[0]}addToPlayer(t,e){let i=this.getPlayer(e);i.push(t);let s=this.getPlayers();s[e]=i,this.setPlayers(s),this.setSelectedIndex(e)}unJoinLeader(t){let e=this.getGroup(t);if(e&&e.length>1){let i=e.filter(e=>e!==t),s=i[0],n=i.filter(t=>t!==s);this.callService("music_assistant","transfer_queue",{entity_id:s,source_player:t}),this.callService("media_player","join",{entity_id:s,group_members:n})}}unJoinFollower(t){this.callService("media_player","unjoin",{entity_id:t})}async unJoinSpeaker(t,e){e&&(this.raiseChangeFlag(),this.isLeader(t)?this.unJoinLeader(t):this.unJoinFollower(t),await this.waitForEntity(t,t=>this.isUnjoined(t,e)),this.lowerChangeFlag(),console.log("done unjoining"))}async joinSpeaker(t,e){this.raiseChangeFlag(),this.callService("media_player","join",{entity_id:e,group_members:[t]}),await this.waitForEntity(t,t=>this.isJoined(t,e)),this.lowerChangeFlag(),console.log("done joining")}handleSelect(t,e){this.setSelectedIndex(e)}handlePointerDown(t,e){this.getSuppressState()||(t.currentTarget.setPointerCapture(t.pointerId),this.createGhost(e),this.setDraggedId(e.speakerId),this.moveGhost(e.clientX,e.clientY))}handlePointerMove(t){this.getSuppressState()||this.moveGhost(t.clientX,t.clientY)}handlePointerUp(t,e){if(!this.getSuppressState()){let i,s=this.getDraggedId();this.removeGhost(),i=t.clientX&&t.clientY?this.renderRoot.elementFromPoint(t.clientX,t.clientY):this.renderRoot.elementFromPoint(t.detail.x,t.detail.y);let n=Array.from(this.renderRoot.querySelectorAll("[data-group-index")).find(t=>t.contains(i)),r=null;n&&(r=parseInt(n.dataset.groupIndex)),this.manipulateSpeaker(s,e,r)}}manipulateSpeaker(t,e,i){e!==i&&(null!==i?this.transferSpeaker(t,i):this.getPlayer(e).length>1&&this.removeSpeaker(t))}async transferSpeaker(t,e){let i=this.removeFromPlayer(t),s=this.getLeaderFromIndex(e);this.addToPlayer(t,e),await this.unJoinSpeaker(t,i),await this.joinSpeaker(t,s)}async removeSpeaker(t){let e=this.removeFromPlayer(t);await this.unJoinSpeaker(t,e)}createGhost(t){this._ghost=document.createElement("speaker-tile"),this._ghost.entityIds=new Set([t.speakerId]),this._ghost.states=this.getStates(),Object.assign(this._ghost.style,{position:"fixed",pointerEvents:"none",opacity:"0.7",zIndex:"1000",width:t.width+"px",height:t.height+"px"}),document.body.appendChild(this._ghost)}moveGhost(t,e){this._ghost&&Object.assign(this._ghost.style,{left:t+"px",top:e+"px",transform:"translate(-50%, -50%)"})}removeGhost(){this._ghost?.remove(),this._ghost=null}_debugLog(t){this._debugMessages=[...this._debugMsgs||[],t]}playerTile(t){if(t.length>0){let e=this.getPlayerIndex(t[0]),i=this.getPlayer(e);return N`
                <speaker-group-panel
                    ...
                    @pointerdown=${t=>this._debugLog("native pointerdown on group-panel")}
                    ...
                    .callService = ${this.callService}
                />`}}render(){if(this.isInitialized()){let t=this.getPlayers();return N`
                <div>
                    ${(this._debugMessages||[]).map(t=>N`<div>${t}</div>`)}
                </div>
            ${tC(t,t=>t[0],t=>this.playerTile(t))}`}}static styles=[tR,r`
